# Backlog Produit 

## EPIC 1 – Compte utilisateur et billets
- Créer un compte
- Connexion à un compte
- Gestion de profil
- Consulter les billets à venir
- Consulter l’historique des billets passés
- Consulter le detail d'un bille

## EPIC 2 – Découverte des événements
- Consulter la liste des événements
- Voir le détail d’un événement
- Découvrir les événements via la carte
- Réserver depuis la map
- Filtrer les evenements
  
## EPIC 3 – Réservation et paiement
- Reserver une place
- Accéder au paiement
- Saisir les informations bancaires
- Confirmer le paiement
- Voir un message de succès


Le backlog détaillé est visible via Jira.


![Backlog jira](https://github.com/blandine-228/PLAZY/blob/main/Assets/backlogProduit.png)




